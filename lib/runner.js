...
var Runner = (function () {
            ...
            _this._evaluator.evaluate(test, function (err) {
                ...
                async.eachSeries(suite.afterEach, function (action, done) { return action.run(done); }, function (err) {
                    if (err)
                        return callback(err);
                    if (_this._baseline) {
                        var hz = _this._getHz(test), baselineHz = _this._baseline.getBaselineHz(test), percentChange;
                    }
                    if (baselineHz) {
    ...
    Runner.prototype._compareTests = function (suite) {
        ...
        tests.forEach(function (test) {
            if (test.pending)
                return;
            var hz = _this._getHz(test), percentSlower = (1 - (hz / fastestHz)) * 100, rank = 0;
     ...
     * @param test The test.
     */
    Runner.prototype._getHz = function (test) {
