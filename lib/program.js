...
program.version(JSON.parse(fs.readFileSync(__dirname + '/../package.json', 'utf8')).version).usage('[options] [files]').option('-b, --baseline <file>', 'full path to file to use for baseline').option('-c, --confidence <percent>', 'percent confidence for reporting changes from baseline [95]').option('-m, --min-time <seconds>', 'minimum time a test runs in seconds [2]').option('-O, --reporter-options <k=v,k2=v2,...>', 'reporter-specific options').option('-R, --reporter <name>', 'specify the reporter to use').option('-t, --timeout <ms>', 'set timeout in milliseconds [60000]').option('-T, --threshold <percent>', 'minimum reported percent change from baseline [5]').option('-u, --update', 'update baseline').option('--colors', 'force enabling of colors').option('--no-colors', 'force disabling of colors').option('--recursive', 'include sub directories').option('--reporters', 'display available reporters');
