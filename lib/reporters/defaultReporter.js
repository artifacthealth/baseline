...
var DefaultReporter = (function (_super) {
    ...
    DefaultReporter.prototype.start = function (timestamp, duration) {
        this.newLine();
        if (timestamp) {
            this.writeLine("Tests will be compared to baseline established on " + timestamp.toLocaleDateString() + " at " + timestamp.toLocaleTimeString() + ".");
            this.newLine();
        }
        if (duration) {
